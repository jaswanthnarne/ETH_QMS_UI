...
const Colleges = () => {
    ...
    const fetchColleges = async () => { try { const res = await axios.get('http://localhost:5000/api/admin/colleges', { headers: { Authorization: `Bearer ${token}` } }); setColleges(res.data.data); } catch (e) { console.error(e); } finally { setLoading(false); } };
    useEffect(() => { fetchColleges(); }, []);
    useSocketUpdate(() => fetchColleges(), ['colleges']);
    const handleSave = async (data) => { try { if (selectedCollege) { await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/admin/colleges/${selectedCollege._id}`, data, { headers: { Authorization: `Bearer ${token}` } }); } else { await axios.post('http://localhost:5000/api/admin/colleges', data, { headers: { Authorization: `Bearer ${token}` } }); } setIsModalOpen(false); fetchColleges(); } catch (error) { alert(error.response?.data?.error || 'Failed to save college'); } };
